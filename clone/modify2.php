...
$query = "SELECT title, contents, user_id, updated_time, file, name from board2 where board_no = $number";
...
<form method="POST" action="modify_action2.php" enctype="multipart/form-data">
...
<tr>
<td>작성자</td>
<td><input type="hidden" name="user_id" value="<?= $userid ?>"><?= $name?></td>
</tr>

<tr>
<td>제목</td>
<td><input type="text" name="title" size=87px; value="<?= $title ?>"></td>
</tr>

<tr>
<td>내용</td>
<td><textarea name="contents"id="contents" cols=75 rows=15><?= $content ?></textarea></td>
</tr>
<input type="file" name="upload" id="upload" value="<?= $file ?>"><?= $file?>
</table>
...
<center>
<input type="hidden" name="board_no" value="<?= $number ?>">
<input style="height:26px; width:47px; font-size:16pxl" type="submit" value="작성">
</center>
