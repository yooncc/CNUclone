...
//ini_set("display_errors",1);
        ...
        session_start();
        $query = "SELECT title, contents, user_id, counts, name from board3 where board_no = $number";
        $result = $connect->query($query);
        $rows = mysqli_fetch_assoc($result);

        $counts = "update board3 set counts = counts + 1 where board_no = $number";
        $connect->query($counts);
					...
					<table class="read_table" align=center>
						...
						<?php
          $query3 = "select * from board3 where board_no='$number'";
          ...
          $idx = $reply['idx']; ?>
				...
				<!--	<input type="text" name="dat_user" id="dat_user" class="dat_user" placeholder="작성자" value=<?= $USER_ID ?>>
					...
					<div class="read_btn">
						<button class="read_btn1" onclick="location.href='./board3.php'">목록</button>&nbsp;&nbsp;
						<button class="read_btn1"
							onclick="location.href='./modify3.php?board_no=<?= $number ?>'">수정</button>&nbsp;&nbsp;
						<button class="read_btn1" a onclick="ask();">삭제</button>
						<script>
							function ask() {
								if (confirm("게시글을 삭제하시겠습니까?")) {
									window.location = "./delete_action3.php?board_no=<?= $number ?>"
								}
							}
						</script>
					</div>
