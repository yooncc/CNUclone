...
<body id="item_body" class="main pc">
	...
	<div class="wrap">
        ...
        $query = "SELECT title, contents, user_id, counts, name from test where board_no = $number";
        $result = pg_query($connect,$query);
        $rows = pg_fetch_assoc($result);

        $query2 = "update test set counts = counts + 1 where board_no = $number";
        $result2 = pg_query($connect,$query2);
					...
					<table class="read_table" align=center>
						...
						<?php
          $query3 = "select * from test where board_no='$number'";
          $result3 = pg_query($connect,$query3);
          $files = pg_fetch_assoc($result3);
          ?>
					...
					<div class="comment_view">
						...
						<?php
          $query4 = "select * from comment where c_no='$number'";
          $result4 = pg_query($connect,$query4);
          while ($reply = pg_fetch_assoc($result4)) {
          ?>
						<div class="dat_view">
							<div><b>
									<?= $reply['name'] ?>
          ...
          $idx = $reply['idx']; ?>
				...
				<!--	<input type="text" name="dat_user" id="dat_user" class="dat_user" placeholder="작성자" value=<?= $USER_ID ?>>
					...
					<div class="read_btn">
						<button class="read_btn1" onclick="location.href='./board4.php'">목록</button>&nbsp;&nbsp;
						<button class="read_btn1"
							onclick="location.href='./modify4.php?board_no=<?= $number ?>'">수정</button>&nbsp;&nbsp;
						<button class="read_btn1" a onclick="ask();">삭제</button>
						<script>
							function ask() {
								if (confirm("게시글을 삭제하시겠습니까?")) {
									window.location = "./delete_action4.php?board_no=<?= $number ?>"
								}
							}
						</script>
					</div>
