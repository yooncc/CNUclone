        ...
        <div class="content" id="jwxe_main_content">
          ...
          <div class="ko board list co-board type01">
            ...
            <div class="college_notice">
              ...
              $query = "SELECT * FROM test ORDER BY board_no DESC"; //역순 출력
              $sql = pg_query($connect, $query);
              //$result = $connect->query($query);
              $total = pg_num_rows($sql); //result set 의 총 레코드(행) 수 반환
              ...
              //게시글 정보 가져오기
              //$result = pg_query($connect, "SELECT * FROM test ORDER BY board_no DESC LIMIT $page_start, $list");
              $result = pg_query($connect, "SELECT * FROM test ORDER BY board_no DESC LIMIT $list offset $page_start");
              ?>


              <!--검색창 시작-->
              <div class="bn-search01 type01">
                <form method="get" action="search_result4.php">
...
<option value="contents">내용</option>
              ...
              <div class="bn-list-common01 type01 bn-common">
                ...
                <table summary="공지사항" class="board-table">
                  ...
                  <tbody>
                    <?php
                    while ($rows = pg_fetch_assoc($result)) { //result set에서 레코드(행)를 1개씩 리턴
                      if ($total % 2 == 0) {
                    ?>
                    <tr class="even">
                      <!--배경색 진하게-->
                    ...
                    <tr>
                      ...
                      <td class="etc" align="left"><a href="read4.php?board_no=<?php echo $rows['board_no'] ?>">
                          <?php echo $rows['title']; ?>
                      </td>
                ...
                <div id="page_num" style="text-align: center;">
                  <?php
                  if ($page <= 1) {
                    echo "<a href='board4.php?page=1'>◀  이전</a>";
                  } else {
                    $pre = $page - 1;
                    echo "<a href='board4.php?page=$pre'>◀  이전</a>";
                  }
                  for ($i = $block_start; $i <= $block_end; $i++) {
                    if ($page == $i) {
                      echo "<b> $i </b>";
                    } else {
                      echo "<a href='board4.php?page=$i'> $i</a>";
                    }
                  }
                  if ($page >= $total_page) {
                    //빈값
                    ...
                    $next = $page + 1;
                    echo "<a href='board.php4?page=$next'>다음  ▶</a>";
                  }
                  if ($page >= $total_page) {
                    //빈 값
                  } else {
                    echo "<a href='board.php4?page=$total_page'> 마지막</a>";
                  }
                  ?>
                </div>
        ...
        <div class=text style="float:right">
          <button style="cursor:hand" onClick="location.href='./write2.php'">글쓰기</button>
        </div>
      </div>
    </div>
  </div>
